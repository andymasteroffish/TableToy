...
//ANDY EDIT
void ofxARToolkitPlus::drawTest(int x, int y) {
    int numDetected = tracker->getNumDetectedMarkers();
    ofSetLineWidth(2);
    ...
    for(int i=0; i<numDetected; i++) {
        ...
        ofCircle(marker.pos[0], marker.pos[1], 8);
        
        ofLine(marker.pos[0], marker.pos[1], marker.vertex[0][0], marker.vertex[0][1]);
        
        ofSetColor( ofColor::yellow );
        ofDrawBitmapString(ofToString( marker.id), marker.pos[0], marker.pos[1]);
        ofDrawBitmapString(ofToString( marker.dir), marker.pos[0], marker.pos[1]+15);
